...
let mySimpleReduce = function(array, cb) {
    let res = array[0];        // start with the first element in the array to avoid multiplying elements in the array with 0
    for (let i = 1; i < array.length; i++) {
        let el = array[i]; 
        res = cb(res, el);      // obtain results by passing the current value and current element in the loop to a callback
    }
    return res;     // return result after loop ends
};
